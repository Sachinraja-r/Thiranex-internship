# ============================================================
# Project Title : Predictive Modeling Using Machine Learning
# Task          : Build a model to predict outcomes based on given data
# Dataset       : Titanic (Seaborn built-in)
# Task Type     : Binary Classification (Survived: 0 or 1)
# Algorithms    : Logistic Regression, Decision Tree, Random Forest
# Tools         : Python, Pandas, NumPy, Matplotlib, Seaborn, Scikit-learn
# ============================================================
#
# NOTE on algorithm choice:
# The spec mentions "Linear Regression" but since we're predicting a
# binary outcome (survived: yes/no), Linear Regression doesn't apply here.
# Logistic Regression is the correct classification equivalent --
# it's basically Linear Regression with a sigmoid function at the end
# that squishes the output into a 0-1 probability. Same idea, right tool.
# Decision Tree and Random Forest are both included as required.
...
import pandas as pd
import matplotlib
matplotlib.use("Agg")  # had to add this because matplotlib was crashing without a display
...
warnings.filterwarnings("ignore")  # suppress those annoying deprecation warnings

# make a folder to save all my graphs
os.makedirs("outputs", exist_ok=True)

# I wanted dark mode charts because they look cooler in presentations
plt.rcParams.update({
    "figure.dpi": 150,
...
# helper function to save plots so i dont have to repeat this every time
...
# seaborn has titanic built in which is super convenient
# -------------------------------------------------------
print("\nLoading data...")
...
# -------------------------------------------------------
# Step 2: Look at the data to understand it
# -------------------------------------------------------
print("\nDataset info:")
print("-" * 40)
print(df.info())
...
# NOTE: mean survival is ~38%, age goes from 0.4 to 80
# also there are missing values in age, embarked, and deck


# -------------------------------------------------------
# Step 3: Clean the data
# This took me the longest to figure out honestly
# -------------------------------------------------------
print("\nCleaning data...")
print("-" * 40)

# dropping columns that either leak the answer or are just repeated info
# - 'alive' is literally the same as 'survived' but as text (thats data leakage!!)
# - 'deck' has way too many missing values (like 77%+ empty)
# - 'who', 'adult_male', 'class', 'alone' are derived from other columns so they're redundant
bad_cols = ["deck", "embark_town", "alive", "who", "adult_male", "class", "alone"]
df.drop(columns=bad_cols, inplace=True)
print(f"  dropped these columns: {bad_cols}")

# fill missing age values with the median
# (using median instead of mean because age might be skewed)
median_age = df["age"].median()
df["age"].fillna(median_age, inplace=True)

# embarked only has 2 missing rows so just fill with most common port
...
# double check no nulls are left
...
# handle outliers in fare using IQR method (learned this in stats class)
...
# encode sex and embarked to numbers because ML models need numbers
le = LabelEncoder()
df["sex_enc"]      = le.fit_transform(df["sex"])       # female=0, male=1
df["embarked_enc"] = le.fit_transform(df["embarked"])  # C=0, Q=1, S=2
df.drop(columns=["sex", "embarked"], inplace=True)
print("  encoded sex and embarked columns")

print(f"\n  final dataset: {df.shape[0]} rows x {df.shape[1]} columns")

# save the cleaned version just in case
...
# Making charts to understand patterns in the data
# -------------------------------------------------------
print("\nMaking EDA charts...")
print("-" * 40)

# chart 1: how many survived vs didn't survive
...
# chart 3: fare distribution (after removing outliers)
fig, ax = plt.subplots(figsize=(9, 5))
...
# chart 5: survival by ticket class
fig, ax = plt.subplots(figsize=(7, 5))
...
# this shows which features are related to each other and to survival
fig, ax = plt.subplots(figsize=(10, 8))
corr = df.corr(numeric_only=True)
mask = np.triu(np.ones_like(corr, dtype=bool))  # hide upper triangle to avoid repeats
...
# chart 7: age vs fare scatter plot (colored by survival)
...
# (decision tree and random forest don't need scaling but lr does)
...
# -------------------------------------------------------
# Step 6: Train 3 different ML models and compare them
#
# KEY FEATURE (from project spec):
#   Apply algorithms like Linear Regression, Decision Trees, or Random Forest
#
# We're using:
#   1. Logistic Regression  -- classification version of linear regression
#   2. Decision Tree        -- tree-based non-linear classifier
#   3. Random Forest        -- ensemble of 200 decision trees (bagging)
# -------------------------------------------------------
print("\nTraining models...")
print("-" * 40)

# model 1 - logistic regression
# good baseline classifier, uses a sigmoid function under the hood
...
# makes a tree of if/else rules to classify data
# max_depth=5 to avoid overfitting (learned that the hard way on an earlier project)
...
# basically a bunch of decision trees that vote together
# n_estimators=200 means 200 trees, should be more accurate than just one tree
...
# Higher accuracy = model is predicting survival correctly more often
# -------------------------------------------------------
print("\nEvaluating models...")
print("-" * 40)
...
def evaluate_model(name, y_true, y_pred):
    # returns a dict with all the metrics i need
    return {
        "Model":     name,
...
#   True Negative  (TN) -- predicted not survived, actually not survived
#   False Positive (FP) -- predicted survived, actually didn't (Type I error)
#   False Negative (FN) -- predicted not survived, actually did (Type II error)
# -------------------------------------------------------
...
# higher AUC = better model at telling survived vs not survived apart
# -------------------------------------------------------
print("\nPlotting ROC curves...")
...
# -------------------------------------------------------
# Step 10: Compare all models with a bar chart
# -------------------------------------------------------
...
# which features actually mattered the most?
# -------------------------------------------------------
...
# -------------------------------------------------------
# Step 12: Detailed classification report for each model
# -------------------------------------------------------
...
print("\n  Key takeaways (Expected Outcome: supervised learning & model evaluation):")
